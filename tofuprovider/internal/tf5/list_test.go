...
type fakeListReponseStream struct {
	grpc.ServerStreamingClient[tfplugin5.ListResource_Event]

	events []*tfplugin5.ListResource_Event
	err    error
}

func (f *fakeListReponseStream) Recv() (*tfplugin5.ListResource_Event, error) {
	if len(f.events) == 0 {
		return nil, io.EOF
	}
	ev := f.events[0]
...
func TestListManagedResourcesImpl(t *testing.T) {
	testProviderCalls(t,
		map[string]providerCallTest[*providerops.ListManagedResourcesRequest, providerops.ListManagedResourcesResponse]{
			"list resource": {
				Mock: func(expect *MockProviderClientMockRecorder) {
					expect.ListResource(
						mockutil.AnyContext(),
						mockutil.Eq(&tfplugin5.ListResource_Request{
							TypeName:              "test_type",
							Limit:                 1234,
							IncludeResourceObject: true,
							Config:                &tfplugin5.DynamicValue{Msgpack: mockutil.MsgPack(map[string]string{"foo": "bar"})},
						}),
					).Return(&fakeListReponseStream{
						events: []*tfplugin5.ListResource_Event{
							{
								DisplayName: "resource1",
								Diagnostic: []*tfplugin5.Diagnostic{
									{
		...
		func(t *testing.T, provider *tf5.Provider, req *providerops.ListManagedResourcesRequest) (providerops.ListManagedResourcesResponse, error) {
