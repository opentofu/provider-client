...
// "package tf6" so it can directly test the unexported implementations of the
...
func TestAttributeImpl(t *testing.T) {
	tests := map[string]struct {
		...
		WantType       cty.Type
		WantNestedType *tfplugin6.Schema_Object
		...
		"structural type": {
			Proto: &tfplugin6.Schema_Attribute{
				...
				NestedType: &tfplugin6.Schema_Object{
					Attributes: []*tfplugin6.Schema_Attribute{
						{Name: "beep", Type: mockutil.JSON("string"), Optional: true},
					},
				},
				Optional: true,
			},
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.WantNestedType != nil {
				if nt := attr.NestedType(); nt != nil {
					implNT, ok := nt.(objectType)
					if !ok {
						t.Errorf("nested type impl is %T, not %T", nt, implNT)
					} else if diff := mockutil.Diff(test.WantNestedType, implNT.proto); diff != "" {
						// We just compare the nested proto directly here,
						// because we have a separate test function to get into
						// the details of how [objectType] is implemented.
						t.Error("wrong structural type:\n" + diff)
					}
				} else {
					t.Errorf("missing expected structural type %#v", test.WantNestedType)
				}
			} else {
				if nt := attr.NestedType(); nt != nil {
					t.Errorf("unexpected structural type: %#v", nt)
				}
			}
...
func TestNestedBlockTypeImpl(t *testing.T) {
	tests := map[string]struct {
		...
		WantAttrs                  map[string]*tfplugin6.Schema_Attribute
		WantBlockTypes             map[string]*tfplugin6.Schema_NestedBlock
	}{
		"single empty": {
			Proto: &tfplugin6.Schema_NestedBlock{
				TypeName: "boop",
		...
		"group empty": {
			Proto: &tfplugin6.Schema_NestedBlock{
				TypeName: "boop",
				Block:    &tfplugin6.Schema_Block{},
				Nesting:  tfplugin6.Schema_NestedBlock_GROUP,
			},
		...
		"attributes": {
			...
			// test our translation of [tfplugin6.Schema_Attribute] more
			// thoroughly.
			Proto: &tfplugin6.Schema_NestedBlock{
				TypeName: "boop",
				Block: &tfplugin6.Schema_Block{
		...
		"nested block types": {
			...
			// where we test our translation of [tfplugin6.Schema_NestedBlock]
			// more thoroughly.
			Proto: &tfplugin6.Schema_NestedBlock{
				TypeName: "boop",
				Block: &tfplugin6.Schema_Block{
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var gotAttrs map[string]*tfplugin6.Schema_Attribute
			for name, attr := range blockType.Attributes() {
				if gotAttrs == nil {
					gotAttrs = make(map[string]*tfplugin6.Schema_Attribute)
				}
			...
			var gotBlockTypes map[string]*tfplugin6.Schema_NestedBlock
			for name, blockType := range blockType.NestedBlockTypes() {
				if gotBlockTypes == nil {
					gotBlockTypes = make(map[string]*tfplugin6.Schema_NestedBlock)
				}
...
func TestObjectTypeImpl(t *testing.T) {
	tests := map[string]struct {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			objTy := providerschema.ObjectType(objectType{
				proto: test.Proto,
			})

			if got, want := objTy.Nesting(), test.WantNestingMode; got != want {
				t.Errorf("wrong nesting mode\ngot:  %s\nwant: %s", got, want)
			}

			var gotAttrs map[string]*tfplugin6.Schema_Attribute
			for name, attr := range objTy.Attributes() {
				if gotAttrs == nil {
					gotAttrs = make(map[string]*tfplugin6.Schema_Attribute)
				}
				implAttr, ok := attr.(attribute)
...
func TestFunctionSignatureImpl(t *testing.T) {
	tests := map[string]struct {
		...
		WantParameters        []*tfplugin6.Function_Parameter
		WantVariadicParameter *tfplugin6.Function_Parameter
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var gotParams []*tfplugin6.Function_Parameter
